...
def salvar(lista,myprograms):
   
    with open(myprograms,"w") as program:
        json.dump(lista,program)
    print(f"Salvo em {myprograms}")


def ler(myprograms):
  with open(myprograms, "r") as program:
    return json.load(program)
...
def menu_():
  ...
  salvar(nomel,"nomes.json")
  salvar(cpfl, "cpf.json")
  salvar(emaill,"email.json")
  salvar(idadel, "idade.json")

  nomel = ler("nomes.json")

  cpfl = ler("cpf.json")

  emaill = ler("email.json")

  idadel = ler("idade.json")
  ...
  if opcoes == "4":
    ...
    nomel.remove(location) 
    cpfl.remove(location)  
    emaill.remove(location) 
    idadel.remove(location) 
